            ...
            {activeTab === 'description' ? (
              <div className="p-5 space-y-5">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="tag font-mono">⏱ {question.timeLimit}ms</span>
                  <span className="tag font-mono">💾 {question.memoryLimit}MB</span>
                ...
                {(question.sampleInput || question.sampleOutput) && (
                  <div className="rounded-xl border border-[var(--border-default)] overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 divide-x divide-[var(--border-subtle)]">
                      <div className="p-3">
                        <p className="mono-label mb-1.5">Input</p>
                        <pre className="text-xs text-accent font-mono whitespace-pre-wrap leading-relaxed">{question.sampleInput || '—'}</pre>
                      </div>
                      <div className="p-3">
                        <p className="mono-label mb-1.5">Output</p>
                        <pre className="text-xs text-info font-mono whitespace-pre-wrap leading-relaxed">{question.sampleOutput || '—'}</pre>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {question.constraints && (
                  <div>
                    ...
                    <div className="bg-[var(--bg-elevated)] rounded-lg border border-[var(--border-subtle)] px-4 py-3">
                ...
                {question.inputFormat && (
                  <div>
                    <p className="mono-label mb-2">Input Format</p>
                    <p className="text-xs text-[var(--text-secondary)] leading-relaxed">{question.inputFormat}</p>
                  </div>
                )}
                ...
                {question.explanation && (
                  <details className="group">
                    <summary className="flex items-center gap-2 cursor-pointer list-none text-xs font-semibold text-[var(--text-muted)] hover:text-accent transition-colors">
                      <svg className="w-3.5 h-3.5 group-open:rotate-90 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg>
                      Show Hint
                    </summary>
                    <div className="mt-3 px-4 py-3 rounded-lg bg-[var(--warn-dim)] border border-[rgba(245,158,11,0.2)] text-xs text-[var(--text-secondary)] leading-relaxed">
                      {question.explanation}
                    </div>
                  </details>
                )}
                ...
                {question.tags?.length > 3 && (
                  <div className="flex flex-wrap gap-1.5">
                    {question.tags.slice(3).map((tag: string) => <span key={tag} className="tag">{tag}</span>)}
                  </div>
                )}
              </div>
              ...
              <div className="p-4 space-y-3">
                ...
                {/* ── Summary bar ── */}
                {totalTests > 0 && (
                  <div className={`rounded-xl border p-3 ${passedTests === totalTests ? 'status-accepted' : 'status-error'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${passedTests === totalTests ? 'bg-accent' : 'bg-danger'}`}>
                          {passedTests === totalTests
                            ? <svg className="w-3 h-3 text-base" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
                        ...
                        <span className="text-xs font-bold">{passedTests === totalTests ? 'All tests passed' : `${passedTests} / ${totalTests} passed`}</span>
                      </div>
                      <span className="font-mono text-xs font-bold">{score}/{maxScore} pts</span>
                    </div>
                    {/* Progress bar */}
                    <div className="progress-bar">
                      <div className="progress-fill" style={{ width: `${totalTests > 0 ? (passedTests / totalTests) * 100 : 0}%` }} />
                    </div>
                  </div>
                )}
                ...
                {output && testCaseResults.length === 0 && (
                  <div className="rounded-xl border border-[var(--border-default)] overflow-hidden">
                    <div className="px-3 py-2 bg-[var(--bg-elevated)] border-b border-[var(--border-subtle)] flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-accent" />
                      <span className="mono-label">stdout</span>
                    </div>
                    <pre className="px-4 py-3 text-xs text-accent font-mono whitespace-pre-wrap leading-relaxed max-h-48 overflow-y-auto">{output}</pre>
                  </div>
                )}
                ...
                {testCaseResults.map((tc, idx) => {
                  ...
                  const isTLE = tc.status === 'timeout';
                  const statusColor = passed ? 'text-accent' : isTLE ? 'text-warn' : 'text-danger';
                  const statusLabel = passed ? 'Accepted' : isTLE ? 'TLE' : tc.status === 'error' ? 'Runtime Error' : 'Wrong Answer';
                  const borderCls = passed
                    ? 'border-[rgba(115,227,59,0.2)]'
                    : isTLE ? 'border-[rgba(245,158,11,0.2)]'
                    : 'border-[rgba(255,71,87,0.2)]';

                  return (
                    <details key={tc.id || idx} className={`rounded-xl border overflow-hidden ${borderCls}`}>
                      <summary className="flex items-center justify-between px-3 py-2.5 cursor-pointer list-none bg-[var(--bg-elevated)] hover:bg-[var(--bg-overlay)] transition-colors">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${passed ? 'bg-accent' : isTLE ? 'bg-warn' : 'bg-danger'}`}>
                            {passed
                        ...
                        <div className="flex items-center gap-3">
                          {tc.executionTime != null && (
                            <span className="text-[10px] font-mono text-[var(--text-muted)]">{tc.executionTime}ms</span>
                          )}
                          <span className={`text-xs font-mono font-bold ${passed ? 'text-accent' : 'text-[var(--text-muted)]'}`}>
                            {tc.pointsEarned ?? 0}/{tc.pointsAvailable ?? 0}pts
                          </span>
                        </div>
                      </summary>
                      ...
                      <div className="bg-[var(--bg-surface)] divide-y divide-[var(--border-subtle)]">
                        ...
                        {tc.input && tc.input !== '[Hidden]' && (
                          <div className="px-3 py-2.5">
                            ...
                            <pre className="text-xs font-mono text-[var(--text-secondary)] whitespace-pre-wrap">{String(tc.input).substring(0, 300)}</pre>
                          </div>
                        )}
                        {/* Expected vs Got — side by side */}
                        {!passed && tc.expectedOutput && tc.expectedOutput !== '[Hidden]' && (
                          <div className="grid grid-cols-2 divide-x divide-[var(--border-subtle)]">
                            <div className="px-3 py-2.5">
                              <p className="mono-label mb-1 text-accent">Expected</p>
                              <pre className="text-xs font-mono text-accent whitespace-pre-wrap">{String(tc.expectedOutput || '').substring(0, 200)}</pre>
                            </div>
                            <div className="px-3 py-2.5">
                              <p className="mono-label mb-1 text-danger">Got</p>
                              <pre className="text-xs font-mono text-danger whitespace-pre-wrap">{String(tc.actualOutput || '(empty)').substring(0, 200)}</pre>
                            </div>
                          </div>
                        )}
                        {/* Error message */}
                        {tc.errorMessage && (
                          <div className="px-3 py-2.5">
                            <p className="mono-label mb-1 text-danger">Error</p>
                            <pre className="text-xs font-mono text-danger/80 whitespace-pre-wrap">{String(tc.errorMessage).substring(0, 400)}</pre>
                          </div>
                        )}
                      </div>
                    </details>
                ...
                {testCaseResults.length === 0 && !output && (
                  <div className="text-center py-14">
                    <div className="w-10 h-10 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-subtle)] flex items-center justify-center mx-auto mb-3">
                      <svg className="w-5 h-5 text-[var(--text-muted)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                    </div>
                    <p className="text-sm font-semibold text-[var(--text-secondary)]">No results yet</p>
                    <p className="text-xs text-[var(--text-muted)] mt-1">Run your code or submit to see output</p>
                  </div>
                )}
              </div>
            )}
